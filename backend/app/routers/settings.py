...
def list_call_logs(
    ...
    if world_id:
        query = query.filter(LLMCallLog.world_id == world_id)
    if sect_id:
        query = query.filter(LLMCallLog.sect_id == sect_id)
    if provider_id:
        query = query.filter(LLMCallLog.provider_id == provider_id)
        ...
        "logs": [
            {
                ...
                "prompt_preview": (l.prompt_preview or "")[:200],
                "success": l.success,
...
def call_log_stats(
    ...
    total_latency = sum(l.latency_ms or 0 for l in logs)
