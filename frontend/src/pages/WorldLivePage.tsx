...
import { Play, CirclePause, Square, Trophy, Wifi, WifiOff } from 'lucide-react';
...
export default function WorldLivePage() {
  ...
  const [activeTab, setActiveTab] = useState<'map' | 'rank' | 'events'>('map');
  ...
  return (
    <div className="h-screen flex flex-col bg-ink-950 text-white overflow-hidden">
      {/* Top Bar - 赛事信息 */}
      <header className="h-14 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border-b border-amber-500/30 flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/')} className="text-slate-400 hover:text-white text-lg">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4 text-sm">
            <div className="flex items-center gap-1.5">
              <Wifi className="w-3.5 h-3.5 text-jade-500 animate-pulse" />
              <span className="text-jade-400">{activeSects.length} 存活</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs font-medium flex items-center gap-1.5 ${liveMode ? 'bg-red-600/30 text-red-300 border border-red-500/50' : 'bg-slate-700 text-slate-400'}`}
            >
              {liveMode ? <><WifiOff className="w-3 h-3" /> 直播中</> : <><CirclePause className="w-3 h-3" /> 暂停</>}
            </button>
          </div>
        </div>
      </header>
      ...
      <div className="flex border-b border-slate-700/50 px-6 shrink-0">
        {([
          { key: 'map', label: '战略地图', icon: Play },
          { key: 'rank', label: '势力榜', icon: Trophy },
          { key: 'events', label: '天道记录', icon: CirclePause },
        ] as const).map(tab => (
          <button
            key={tab.key}
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-xs px-1.5 py-0.5 rounded ${
                        sect.status === 'active' ? 'bg-jade-900/50 text-jade-400' : 'bg-red-900/50 text-red-400'
                      }`}>
        ...
        {activeTab === 'rank' && (
          <div className="flex-1 overflow-y-auto p-6">
            <div className="max-w-3xl mx-auto">
              <h2 className="text-xl font-bold text-amber-200 mb-6 flex items-center gap-2">
                <Trophy className="w-5 h-5" /> 宗门势力排行
              </h2>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className={`text-2xl font-bold w-8 text-center ${idx === 0 ? 'text-gold-400' : idx === 1 ? 'text-slate-300' : idx === 2 ? 'text-amber-700' : 'text-slate-500'}`}>
                          {idx + 1}
                        </span>
                        <div>
                          <div className="font-medium text-lg" style={{ color: SECT_COLORS[sect.sect_type] }}>
                            {sect.name}
                          </div>
                          <div className="text-xs text-slate-500">掌门: {sect.leader_name || '未知'}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-6 text-sm">
                        <div className="text-center">
                          <div className="text-slate-500 text-xs">战力</div>
                          <div className="font-bold">{sect.stats.military_power || 0}</div>
                        </div>
                        <div className="text-center">
                          <div className="text-slate-500 text-xs">区域</div>
                          <div className="font-bold">{sect.controlled_regions.length}</div>
                        </div>
                        <div className="text-center">
                          <div className="text-slate-500 text-xs">灵石</div>
                          <div className="font-bold">{sect.resources.spirit_stones || 0}</div>
                        </div>
        ...
        {activeTab === 'events' && (
          <div className="flex-1 overflow-y-auto">
            <div className="max-w-3xl mx-auto p-6">
              <h2 className="text-xl font-bold text-amber-200 mb-6">📜 天道记录</h2>
              <div className="space-y-2">
                {timelineItems.map((item, idx) => (
                  <div key={`${item._type}-${idx}`} className="p-3 rounded-lg bg-slate-800/50 border border-slate-700/30">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs text-slate-500">回合 {item.turn}</span>
                      {item._type === 'battle' ? (
                        <span className="text-xs px-1.5 py-0.5 rounded bg-red-900/50 text-red-400">⚔️ 战争</span>
                ...
                {timelineItems.length === 0 && (
                  <div className="text-center text-slate-600 py-12">暂无事件</div>
                )}
              </div>
            </div>
          </div>
